    ...
    try:
        ...
        if events:
            ...
            for event in reversed(events):
                ...
                try:
                    # Envolver en lista para el Batch API
                    res = requests.post(API_URL, json=[payload], timeout=3)
                    nombre_exe = payload['proceso'].split('\\')[-1]
                    
                    resp_json = res.json()
                    
                    # Validar si hubo un error de formato (ej. 422)
                    if res.status_code != 200:
                        print(f"\n[-] Error de API (HTTP {res.status_code}): {resp_json}")
                        continue
                    
                    # Extraer del nuevo formato "results"
                    if "results" in resp_json and len(resp_json["results"]) > 0:
                        resultado = resp_json["results"][0]
                        prob_edr = resultado.get('prob_edr_cruda', 0.0)
                        prob_fusion = resultado.get('prob_fusion_xdr', 0.0)
                        accion = resultado.get('accion', 'PERMITIR')
                    else:
                        prob_edr, prob_fusion, accion = 0.0, 0.0, 'DESCONOCIDA'
                    ...
                    print(f"\n[-] Fallo de conexión con la API: {e}")
